...
Due to errors we had in class, I put the updated verson of `src/database.py` in the repo. Please review the code and make sure you understand how the connection to the MySQL database is established using SQLAlchemy.
...
**Note**: Due to problems we had in class, I have already set up Alembic for you

You can just see the `migrations` folder. 

##### 2.1 — Generate the first migration
...
1. Fetch all columns for every student and print their first_name.
  every_student = session.get(Student)
  for i in every_student
  print(i.first_name)

2. Find the single student who has the email 'test@example.com'.
  stmt = select(Student).where(Student.email == 'test@example.com')
  session.scalar(stmt)
