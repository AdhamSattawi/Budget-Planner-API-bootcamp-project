...
└── e2e/
    ├── e2e_utils.py           # Shared helpers: server lifecycle, TestResults
    ├── students_e2e.py        # CRUD e2e tests for /students/
    └── run_all.py             # Runs all suites; prints combined summary
```
...
The `e2e/` directory contains end-to-end scripts that start the server on port `10110`, send real HTTP requests, and stop the server when done.

**Prerequisites:** database must be running with migrations applied (`alembic upgrade head`).

Assuming you activated the virtual environment, you can run the e2e tests with:

```bash
# Run a single suite
python e2e/students_e2e.py

# Run all suites with a combined summary
python e2e/run_all.py
```

Each script creates the entities it needs via the API and deletes them at the end — scripts are idempotent and can be run repeatedly.
